  ...
  <div class="exam-page">
    ...
    <div class="page-header">
      <div>
        ...
        <p>View and monitor examinations created by all faculty.</p>
      </div>
    </div>
    ...
    <div class="exam-section">
      <div class="section-header">
        <div>
          ...
          <p>{{ filteredExams.length }} examination(s) displayed</p>
        </div>
            ...
            <tr v-for="exam in filteredExams" :key="exam.id">
              ...
              <td>
                <div class="faculty-info">
                  ...
                  <div>
                    <strong>{{ exam.faculty?.name || 'Unknown Faculty' }}</strong>
                    <small>{{ exam.faculty?.email || 'No email' }}</small>
                  </div>
                </div>
              </td>
              ...
              <td>
                <div class="grade-section">
                  ...
                  <small v-if="exam.section">{{ exam.section }}</small>
                </div>
              </td>
...
const totalQuestions = computed(() => {
  return exams.value.reduce(
    (total, exam) => total + Number(exam.questions_count || 0),
    0
  )
})

const totalExaminees = computed(() => {
  return exams.value.reduce(
    (total, exam) => total + Number(exam.students_count || 0),
    0
  )
})

const facultyCount = computed(() => {
  const ids = exams.value
    .map(exam => exam.faculty?.id)
...
const facultyOptions = computed(() => {
  ...
  exams.value.forEach(exam => {
    if (exam.faculty?.id !== null && exam.faculty?.id !== undefined) {
...
const statusOptions = computed(() => {
  const statuses = exams.value
    .map(exam => exam.status?.trim().toLowerCase())
...
const filteredExams = computed(() => {
  ...
  return exams.value.filter(exam => {
    const matchesSearch =
      ...
      exam.grade?.toLowerCase().includes(keyword) ||
      exam.section?.toLowerCase().includes(keyword) ||
      exam.faculty?.name?.toLowerCase().includes(keyword) ||
      exam.faculty?.email?.toLowerCase().includes(keyword)

    const matchesFaculty =
      !facultyFilter.value ||
      String(exam.faculty?.id) === facultyFilter.value

    const matchesSubject =
      !subjectFilter.value ||
      exam.subject === subjectFilter.value

    const matchesStatus =
      !statusFilter.value ||
      exam.status?.toLowerCase() === statusFilter.value
...
async function fetchExams() {
  ...
  try {
    ...
    if (error.response?.status === 401) {
      ...
      errorMessage.value = 'Failed to load examinations.'
    }
  } finally {
    loading.value = false
  }
